...
def split_targets(rows):
    ...
    for row in rows:
        ...
        if lane == 'critic':
            ...
            if meta['estimator'] not in ('refined_td', 'direct_branch_td', 'vine_ppo'):
...
def zero_advantage_placeholder(actors):
    """Retain one masked span when an entire question has zero policy signal.

    Slime's native scheduler requires every question ID in the six-question
    global batch.  A fully masked real span contributes exactly zero policy and
    KL loss while preserving that ID.  Choosing the shortest span minimizes the
    otherwise wasted forward-pass work.
    """
