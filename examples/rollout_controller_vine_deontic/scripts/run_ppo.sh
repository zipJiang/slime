...
storage_root=${PPO_STORAGE_ROOT:-/weka/projects/bvandur1/zjiang31/deontic-vine-9b/runs}
...
export RAY_ADDRESS=${RAY_ADDRESS:-172.16.203.14:6428}
...
exec bash "$experiment_root/scripts/sif.sh" python "$experiment_root/scripts/with_torch_cudnn.py" \
  python "$experiment_root/scripts/train_vine.py" \
  "${MODEL_ARGS[@]}" \
  --actor-num-nodes "${PPO_TRAIN_NODES:-1}" --actor-num-gpus-per-node "${PPO_TRAIN_GPUS_PER_NODE:-4}" --rollout-num-gpus "${PPO_ROLLOUT_GPUS:-6}" \
  ...
  --num-rollout 120 --num-critic-only-steps 0 --rollout-batch-size 6 --n-samples-per-prompt 1 \
  --num-steps-per-rollout 1 --global-batch-size 6 --eval-interval 120 --skip-eval-before-train \
  --eval-prompt-data airline "$experiment_root/data/test-airline.jsonl" \
    sara_numeric "$experiment_root/data/test-sara_numeric.jsonl" \
    sara_binary "$experiment_root/data/test-sara_binary.jsonl" \
  ...
  --seq-length 32768 --use-dynamic-batch-size --max-tokens-per-gpu 16384 --log-probs-max-tokens-per-gpu 16384 --balance-data \
  ...
  --use-wandb --wandb-mode online --wandb-project deontic-vine-ppo \
