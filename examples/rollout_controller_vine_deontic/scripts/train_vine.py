...
def custom_args(parser):
    ...
    parser.add_argument('--vine-value-rollouts-per-state', type=int, default=1)
    parser.add_argument('--resume-vine-value-rollouts-from', type=int)
    ...
    parser.add_argument('--ppo-seed-namespace', default='vine-balanced-base-sep16')
    return parser
...
def train(args):
    ...
    if args.n_samples_per_prompt != 1 or args.global_batch_size != args.rollout_batch_size or args.num_steps_per_rollout != 1:
        raise ValueError('Expected one optimizer step per fresh six-question batch')
    ...
    try:
        for round_id in range(start, args.num_rollout):
            ...
            status = dict(stage='vine_ppo', completed_actor_updates=round_id+1,
                ...
                group_size=args.vine_group_size,
                value_rollouts_per_state=args.vine_value_rollouts_per_state,
