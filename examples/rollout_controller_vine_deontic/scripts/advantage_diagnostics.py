...
def summarize(records):
    ...
    if not edges or not total_tokens:
        raise ValueError('No actor signal population')
    ...
    for threshold in thresholds:
        ...
        report[f'abs_le_{key}_edge_fraction'] = sum(abs(v) <= threshold for v in edges.values()) / len(edges)
        report[f'abs_le_{key}_token_fraction'] = token_counts[threshold] / total_tokens
    return report
