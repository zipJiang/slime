...
from recipe import RECIPE_ID, RESUME_COMPATIBLE_RECIPE_IDS
...
def prepare_resume(args):
    ...
    if source_recipe_id not in RESUME_COMPATIBLE_RECIPE_IDS:
        raise ValueError('Resume recipe is not explicitly compatible with this VinePPO recipe')
    ...
    previous_value_rollouts = previous.get('vine_value_rollouts_per_state', 1)
    current_value_rollouts = getattr(args, 'vine_value_rollouts_per_state', 1)
    value_rollout_change = None
    authorized_value_rollouts = getattr(args, 'resume_vine_value_rollouts_from', None)
    if (authorized_value_rollouts is not None
            and authorized_value_rollouts != previous_value_rollouts):
        raise ValueError('Authorized source value rollout count does not match checkpoint')
    if previous_value_rollouts != current_value_rollouts:
        if authorized_value_rollouts is None or current_value_rollouts < 1:
            raise ValueError('Resume changes training setting: vine_value_rollouts_per_state')
        value_rollout_change = dict(
            previous=previous_value_rollouts,
            current=current_value_rollouts)
    ...
    return dict(iteration=iteration, start_rollout_id=start, actor_updates=start,
                ...
                group_size_change=group_change, source_recipe_id=source_recipe_id,
                value_rollout_change=value_rollout_change,
                current_recipe_id=RECIPE_ID,
                recipe_change=(None if source_recipe_id == RECIPE_ID else
                               dict(previous=source_recipe_id, current=RECIPE_ID)))
