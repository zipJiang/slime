...
class TargetTests(unittest.TestCase):
    ...
    def test_zero_placeholder_is_shortest_real_span_with_no_loss(self):
        rows = [
            dict(tokens=[1, 2, 3], response_length=2, loss_mask=[1, 1],
                 rollout_log_probs=[-.2, -.3], reward=0., group_index=4,
                 metadata=dict(node_id=7, edge_loss_scale=1., lane='actor')),
            dict(tokens=[1, 2], response_length=1, loss_mask=[1],
                 rollout_log_probs=[-.4], reward=0., group_index=4,
                 metadata=dict(node_id=8, edge_loss_scale=1., lane='actor')),
        ]
        placeholder = zero_advantage_placeholder(rows)
        self.assertEqual(placeholder['tokens'], [1, 2])
        self.assertEqual(placeholder['loss_mask'], [0])
        self.assertEqual(placeholder['rollout_log_probs'], [-.4])
        self.assertEqual(placeholder['metadata']['filtered_zero_edges'], 2)
        self.assertTrue(placeholder['metadata']['zero_advantage_placeholder'])
        self.assertEqual(placeholder['metadata']['edge_loss_scale'], 0.)
        self.assertEqual(rows[1]['loss_mask'], [1])
