"""Run fresh LocBench GRPO on pinned learner and rollout reservations."""
import argparse
import datetime
import json
import os
    ...
    try:
        ...
        for job, devices in assignments.items():
            ...
            host_text = subprocess.check_output(
                step(job, "loc-grpo-host", 1, ["hostname", "-I"]), env=env, text=True
            )
            hosts[job] = next(ip for ip in host_text.split() if ip.startswith("172."))
            query = subprocess.check_output(
                step(job, "loc-grpo-free", 1, [
            ...
            busy = {device: memory.get(device) for device in devices if memory.get(device, 999999) > 100}
            if busy:
                raise ValueError(f"Selected GPUs are occupied on {job}: {busy}")
        ...
        while driver.poll() is None:
            if any(child.poll() is not None for name, child in children.items() if name.startswith("ray-")):
                raise RuntimeError("Ray service exited")
            if run.exists() and (run / "recipe.json").exists() and audit is None:
            ...
            if audit is not None and audit.poll() not in (None, 0):
                (run / "STOP").touch()
                raise RuntimeError("Checkpoint audit failed")
            if time.time() > drain_at and run.exists():
                (run / "STOP").touch()
            status = {"stage": "training", "time": time.time(), "run": str(run), "drain_at": drain_at}
        ...
        if audit is not None and audit.wait(timeout=1800):
            raise RuntimeError("Checkpoint audit failed")
        result = run / ("completed.json" if (run / "completed.json").exists() else "paused.json")
