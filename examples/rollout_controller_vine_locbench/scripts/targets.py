...
def split_targets(rows):
    ...
    for row in rows:
        ...
        if lane == 'critic':
            ...
            if meta['estimator'] not in ('refined_td', 'direct_branch_td', 'vine_ppo', 'grpo'):
...
def zero_advantage_placeholder(actors):
    """Keep a question in the native batch without computing a zero loss.

    GRPO gives every edge in an all-equal outcome group exactly zero policy
    advantage.  Slime still requires every question ID in the four-question
    global batch, so retain the shortest conditioning span with a fully masked
    response.  It contributes neither policy nor KL loss.
    """
    if not actors or any(float(row['reward']) != 0.0 for row in actors):
        raise ValueError('A zero placeholder requires a nonempty all-zero actor group')
    source = min(actors, key=lambda row: (len(row['tokens']), row['response_length']))
    placeholder = dict(source)
    placeholder['tokens'] = list(source['tokens'])
    placeholder['loss_mask'] = [0] * len(source['loss_mask'])
    placeholder['rollout_log_probs'] = list(source['rollout_log_probs'])
    placeholder['reward'] = 0.0
    placeholder['metadata'] = dict(source['metadata'])
    placeholder['metadata'].update(
        zero_advantage_placeholder=True,
        filtered_zero_edges=len({row['metadata']['node_id'] for row in actors}),
        edge_loss_scale=0.0,
    )
    return placeholder
