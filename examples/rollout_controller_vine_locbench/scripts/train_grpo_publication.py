"""Actor-only step-wise GRPO with one-batch asynchronous rollout overlap."""
import copy
import hashlib
import json
import os
...
from resume_grpo import prepare_resume, validate_actor_restore
from optimizer_restore import inspect_optimizer
from vine_actor_publication import VineActor
...
def custom_args(parser):
    ...
    parser.add_argument('--ppo-seed-namespace', default='locbench-grpo-imitation-sep20')
    parser.add_argument('--loc-collection-profile', default='compact24-reply8-read60')
    parser.add_argument('--grpo-calibration-only', action='store_true')
    return parser
...
def train(args):
    ...
    if args.colocate or args.release_train or args.num_critic_only_steps:
        raise ValueError('GRPO uses dedicated persistent actor ranks and no critic warmup')
    if args.normalize_advantages or args.calculate_per_token_loss:
        raise ValueError('Keep prepared GRPO credit and question/edge normalization')
    ...
    if args.rollout_temperature != 1.0 or args.log_probs_chunk_size <= 0 or not args.bf16:
        raise ValueError('Chunked GRPO logits require BF16, temperature1, and positive chunk size')
    ...
    # training GPUs with a learned critic. GRPO has no second model on those GPUs.
    ...
    def begin(round_id):
        frozen = dict(rollout_id=round_id, policy_version=f'actor-{behavior_round:04d}',
            ...
            value_version='none', value_url='', estimator='grpo',
    ...
    try:
        if args.grpo_calibration_only:
            ...
            report = dict(group_size=args.grpo_group_size, batches=batches,
        ...
        for round_id in range(start, args.num_rollout):
            ...
            status = dict(stage='grpo', completed_actor_updates=round_id+1,
                execution=args.ppo_execution, lineage=lineage, prefetched_next=ready is not None,
                group_size=args.grpo_group_size, checkpoint_saved=save,
