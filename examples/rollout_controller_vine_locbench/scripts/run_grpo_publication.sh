...
checkpoint=${LOC_GRPO_ACTOR_CHECKPOINT:-/weka/projects/bvandur1/zjiang31/locbench-9b/runs/imitation-24k-read60-lr1e6-v1/hf/iter_0000005}
run_root="$experiment_root/runs/${GRPO_RUN_NAME:?Set a fresh GRPO_RUN_NAME}"
storage_root=${GRPO_STORAGE_ROOT:-/weka/projects/bvandur1/zjiang31/locbench-grpo-9b/runs}
mkdir -p "$experiment_root/runs" "$storage_root/$GRPO_RUN_NAME"
if [[ ! -e "$run_root" ]]; then
  ln -s "$storage_root/$GRPO_RUN_NAME" "$run_root"
fi
export SLIME_EXTRA_PYTHONPATH="$experiment_root/scripts"
export RAY_ADDRESS=${RAY_ADDRESS:?Set the dedicated LocBench GRPO Ray address}
...
exec bash "$experiment_root/scripts/sif.sh" python "$experiment_root/scripts/with_torch_cudnn.py" \
  python "$experiment_root/scripts/train_grpo_publication.py" \
  "${MODEL_ARGS[@]}" \
  --actor-num-nodes 1 --actor-num-gpus-per-node 4 --rollout-num-gpus "${GRPO_ROLLOUT_GPUS:-6}" \
  ...
  --rollout-function-path grpo_slime_shim.generate_rollout \
  --custom-convert-samples-to-train-data-path grpo_slime_shim.convert_actor_data \
  ...
  --num-rollout "${GRPO_NUM_ROLLOUT:-97}" --num-critic-only-steps 0 \
  ...
  --grpo-group-size "${GRPO_GROUP_SIZE:-8}" --ppo-search-concurrency "${GRPO_SEARCH_CONCURRENCY:-12}" \
  --ppo-execution overlap --loc-collection-profile compact24-reply8-read60 \
  --use-wandb --wandb-mode online --wandb-project locbench-grpo \
  --disable-wandb-random-suffix --wandb-group "$GRPO_RUN_NAME" --wandb-dir "$run_root/wandb" \
  "$@"
