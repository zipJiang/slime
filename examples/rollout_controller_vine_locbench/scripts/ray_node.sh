...
export CUDA_VISIBLE_DEVICES=$devices RAY_TMPDIR=/tmp/locbench-vine-ray-v1
...
if [[ $role == train ]]; then
  ray_args=(--head --port=6438 --dashboard-port=8318)
else
...
exec bash "$experiment_root/scripts/sif.sh" python "$experiment_root/scripts/with_torch_cudnn.py" \
  ...
  --dashboard-agent-listen-port=54331 --runtime-env-agent-port=54332 \
  --resources="{\"locbench_vine_$role\":$count}" \
