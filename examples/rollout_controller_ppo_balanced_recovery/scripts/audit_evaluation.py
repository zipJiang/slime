...
from balanced_data import EVAL_SEED, test_case_keys, metadata_for
...
def audit(directory, branches=1):
    ...
    assert contract['seed_namespace'] == EVAL_SEED
    ...
    cases, case_keys, split_hash = corpus_and_split(SimpleNamespace(
        questions=directory/'questions.json', split='val', evaluation=True))
    expected = test_case_keys()
    assert case_keys == contract['case_keys'] == expected
    ...
    assert summary['branches'] == len(case_keys) * branches
    ...
    for group, case_key in enumerate(case_keys):
        for branch in range(branches):
            ...
            assert (result['group_index'], result['case_key'], result['branch']) == (group, case_key, branch)
            final = pickle.loads(gzip.decompress(native_bytes))
            assert result['family'] == metadata_for(case_key)['family']
            assert result['hard'] == metadata_for(case_key)['hard']
            trace = audit_trace(final, result, cases[case_key], contract['policy_version'])
            traces.append(dict(group_index=group, case_key=case_key, branch=branch, **trace))
