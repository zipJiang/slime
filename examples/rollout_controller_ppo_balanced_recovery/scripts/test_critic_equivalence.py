...
class CriticEquivalenceTests(unittest.TestCase):
    ...
    def test_explicit_best_effort_allowance_still_bounds_error(self):
        native = [.4901745915412903]
        replica = [.4851118326187134]
        self.assertFalse(self.check(native, replica)['passed'])
        report = self.check(native, replica, tolerance=.01)
        self.assertTrue(report['passed'])
        self.assertAlmostEqual(report['mean_abs_error'], .005062758922576904)
        self.assertFalse(self.check([.5], [.489], tolerance=.01)['passed'])
        self.assertFalse(self.check([.5], [.5], [.50001], tolerance=.01)['passed'])
