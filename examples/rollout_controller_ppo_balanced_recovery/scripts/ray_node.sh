...
role=${1:?train, train_worker, or rollout}
address=${2:-}
rollout_gpus=${3:-4}
export RAY_TMPDIR=/tmp/deontic-balanced-ray
...
if [[ "$role" == train || "$role" == train_worker ]]; then
  if [[ "$role" == train ]]; then
    ray_args=(--head --port=6425 --dashboard-port=8305)
  else
    ray_args=(--address="${address:?head address}")
  fi
  ray_args+=(--num-gpus="$rollout_gpus" --num-cpus=32
    --resources="{\"deontic_direct_branch_train\":$rollout_gpus}")
else
