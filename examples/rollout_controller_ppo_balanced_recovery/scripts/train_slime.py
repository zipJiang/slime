...
def custom_args(parser):
    ...
    parser.add_argument('--ppo-critic-equivalence-tolerance', type=float, default=.005)
    parser.add_argument('--ppo-seed-namespace')
    parser.add_argument('--ppo-stop-after-round', type=int)
    parser.add_argument('--ppo-preflight-only', action='store_true')
...
def train(args):
    ...
    if resume is not None and resume['actor_updates'] > 0:
        from optimizer_restore import inspect_optimizer, validate_restore
        restored = {role:ray.get([handler.__ray_call__.remote(inspect_optimizer)
                    for handler in group._actor_handlers])
                    for role,group in [('actor',actor),('critic',critic)]}
        write_json(run/'initial-native-restore-audit.json',
            validate_restore(resume,restored,args.global_batch_size))
    ...
    def publish_critic(completed_rounds):
        ...
        comparison = compare_scores(expected, actual, repeated, version=version,
                                    count=len(check_contexts), tolerance=args.ppo_critic_equivalence_tolerance)
    ...
    try:
        ...
        for round_id in range(args.start_rollout_id, last_round+1):
            ...
            if (directory/'warmup-replay-source.json').exists():
                next_source = Path(args.ppo_replay_warmup_run)/'rollouts'/f'train-{round_id+1:04d}'/'summary.json'
                # Persist the rebuilt optimizer and advanced question cursor
                # before spending compute on the first fresh recovery batch.
                save_now = save_now or not next_source.exists()
            ...
            replay_provenance = directory/'warmup-replay-source.json'
            if replay_provenance.exists():
                source_audit = Path(args.ppo_replay_warmup_run)/'critic-publication'/f'critic-{round_id+1:04d}.json'
                expected = json.loads(source_audit.read_text())
                if expected['context_sha256'] != [hashlib.sha256(c.encode()).hexdigest() for c in equivalence_contexts]:
                    raise ValueError('Recovery comparison contexts changed')
                version = expected['version']
                native_scorer.begin(version)
                try:
                    restored = native_scorer.score(equivalence_contexts, version)
                finally:
                    native_scorer.end()
                from warmup_replay import compare_optimizer_replay
                comparison = compare_optimizer_replay(expected['native'], restored,
                    tolerance=args.ppo_optimizer_replay_tolerance)
                write_json(directory/'optimizer-replay-audit.json', dict(**comparison,
                    expected=expected['native'], restored=restored,
                    source=str(source_audit), optimizer_updates=round_id+1))
                if not comparison['passed']:
                    raise RuntimeError('Replayed critic exceeds the configured restart tolerance')
...
if __name__ == '__main__':
    ...
    if args.ppo_preflight_only:
        actor, critic, resume = role_arguments(args)
        if args.actor_num_nodes * args.actor_num_gpus_per_node != 4 or args.tensor_model_parallel_size != 2:
            raise ValueError('Resume must retain four training ranks with TP=2')
        print(json.dumps(dict(passed=True, resume=resume,
            actor_no_load_optim=actor.no_load_optim, critic_no_load_optim=critic.no_load_optim,
            train_nodes=args.actor_num_nodes, train_gpus_per_node=args.actor_num_gpus_per_node,
            rollout_gpus=args.rollout_num_gpus), default=str), flush=True)
        raise SystemExit(0)
