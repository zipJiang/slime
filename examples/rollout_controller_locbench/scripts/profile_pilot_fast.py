...
async def run(args):
    ...
    manifest=dict(protocol='locbench-compaction-behavior-pilot-v1',questions=QUESTIONS,samples=2,execution='parallel arms, direct engine endpoints; second seed uses verified additional decoders if ready',
    ...
    async with httpx.AsyncClient(timeout=1800.,limits=httpx.Limits(max_connections=32)) as http:
        ...
        async def wave(sample):
            ...
            for i,key in enumerate(jobs):assignments[key]=endpoints[i%len(endpoints)]
            ...
            return rows
        waves=await asyncio.gather(wave(0),wave(1),return_exceptions=True)
