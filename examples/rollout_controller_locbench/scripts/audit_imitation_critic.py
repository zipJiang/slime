...
from runtime_v2 import EXPERIMENT, MODEL, CONTEXT_LIMIT, context, digest, verify_harness
from synthetic_runtime import environment as contract,LimitedRead
from imitation_lineage import require_model
from step_controller.harness.tools.base import tool_schema
...
def audit(collection,expected,complete=False):
    ...
    identity=require_model(manifest['actor_identity']['training'])
    if manifest['actor_identity']!=identity:raise ValueError('Actor identity changed')
    ...
    for lane in ['train','validation']:
        for path in (collection/lane).glob('*/sample-*.json'):
            ...
            tools=[tool_schema(LimitedRead(repo)) if t.name=='read' else tool_schema(t) for t in LocBenchEnv(repo,case.gold,call_budget=20,maximum=10).tools]
