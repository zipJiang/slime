...
from runtime import EXPERIMENT, MODEL, CONTEXT_LIMIT, TASK_LIMIT, REPLY_LIMIT, contract, context, digest, make_world, verify_harness
...
async def run(args):
    ...
    manifest=dict(protocol='locbench-mc-critic-v1',environment=contract(),
        ...
        samples_per_question=args.samples,seed_namespace='locbench-critic-mc-v1',
        ...
        sources={str(p):digest(p) for p in [Path(__file__).resolve(),EXPERIMENT/'scripts/runtime.py']})
    ...
    policies=[MonteCarloPolicy(model=MODEL,served_model='locbench-qwen35-9b',format=format,
        base_url=url,api_key='EMPTY',timeout=1800,version='locbench-base-9b-mc-v1',
    ...
    async def one(index,lane,q,sample):
        ...
        async with gates[endpoint]:
            ...
            draw_token=DRAW.set(saved['draw'] if saved else (f'locbench-critic-mc-v1/{lane}/{q}/{sample}',0))
