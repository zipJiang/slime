...
class BenchmarkTests(unittest.TestCase):
    def arms(self):
        ...
        base = dict(execution='sync', resume={'iteration':5}, total_gpus=10,
            recipe_arguments=arguments, tokens_per_second=100, output_tokens=6000000,
            rows=[dict(round=i, families=['a','b'], pass_tokens=140000,
                       seed_namespace=f'paired/{i}') for i in (6,7,8)])
        candidate = copy.deepcopy(base)
        candidate.update(execution='overlap', tokens_per_second=130)
        return base, candidate

    def test_requires_real_speedup_and_comparable_budget(self):
        a,b = self.arms()
        self.assertTrue(compare(a,b)['prioritize_overlap'])
        b['tokens_per_second'] = 115
        self.assertFalse(compare(a,b)['prioritize_overlap'])
        b['tokens_per_second'] = 140
        b['output_tokens'] *= 1.3
        self.assertFalse(compare(a,b)['prioritize_overlap'])
