    ...
    for group, family in enumerate(contract['families']):
        ...
        if contract.get('horizon') == 'shared_task_turns_plus_one_forced_submission':
            for node in state.nodes.values():
                p = node.payload
                assert p.turns_taken <= contract['max_steps'] + 1, f'{family}: horizon exceeded'
                if p.turns_taken > contract['max_steps']:
                    assert p.done or p.truncated, f'{family}: live checkpoint past horizon'
        ...
        for expected, lane in ((actor, 'actor'), (critic, 'critic')):
            ...
            assert len(actual) == len(expected), f'{family}: {lane} record count'
            for a, b in zip(actual, expected, strict=True):
                a = dict(a, metadata=dict(a['metadata']))
                for key, value in dict(family=family, policy_version=contract['policy_version'],
                    server_weight_version=contract['server_weight_version']).items():
                    assert a['metadata'].pop(key) == value, f'{family}: {key}'
                ...
                assert a == b, f'{family}: {lane} replay differs at node {b["metadata"]["node_id"]}'
        ...
        assert cost == result['cost'], f'{family}: generated token accounting mismatch'
        ...
        reports.append(dict(family=family, actor_spans=len(actor), critic_checkpoints=len(critic),
